...
newtype AsyncHandler m a =
    AsyncHandler (TBMQueue (a, T.Text, Maybe (MVar ())) -> m ())
...
class (MonadMatrixBot m) => MonadSyncGroupManager m where
    ...
    gcSyncGroups :: m ()
    default gcSyncGroups
        :: (m ~ m1 m2, MonadTrans m1, MonadSyncGroupManager m2) => m ()
...
instance (MonadMatrixBotBase m, MonadResyncableMatrixBot m, MonadUnliftIO m)
    ...
    gcSyncGroups = SyncGroupManagerT $ do
        ended <- get >>= liftIO . fmap catMaybes
            . traverse (\(thread, rk) -> fmap (thread, rk, ) <$> poll thread)
            . M.toList
        mapM_ (\(_, rk, _) -> release rk) ended
        modify $ \allThreads ->
            foldl' (\acc (thread, _, _) -> M.delete thread acc)
                   allThreads
                   ended
...
syncGroupHandler mkS handler = AsyncHandler $ \queue -> do
  ...
  where
    startHandler lastSeenSyncTokenRef queue =
        catch (mkS >>= go lastSeenSyncTokenRef queue)
              (restart lastSeenSyncTokenRef queue)

    restart lastSeenSyncTokenRef queue e = case fromException e of
        ...
        Nothing -> do
            ...
            resyncAt <- liftIO $
                discardUntilNextSyncToken lastSeenSyncTokenRef queue
...
resourceTAsync
    :: (MonadResource m) => UnliftIO n -> n a -> m (ReleaseKey, Async a)
...
killThread thread = do
    ...
    case result of
        Left e -> case fromException e of
            Just AsyncCancelled -> pure ()
            Nothing -> logStderr $
                "SyncGroup threw an unexpected exception: " ++ show e
        Right _ -> pure ()
    pure ()
