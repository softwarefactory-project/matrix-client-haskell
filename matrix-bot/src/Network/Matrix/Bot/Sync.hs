...
module Network.Matrix.Bot.Sync ( syncLoop, getInitialSyncToken ) where
...
import           Network.Matrix.Client
                     ...
                     , MatrixError(MatrixError, meErrcode)
                     , MatrixM
                     , UserID
                     , createFilter
                     , defaultEventFilter
...
newtype SyncTokenAccountData = SyncTokenAccountData { stadSyncToken :: T.Text }
...
getInitialSyncToken
    :: (MonadIO m) => ClientSession -> UserID -> MatrixM m (Maybe T.Text)
getInitialSyncToken session
                    userID = liftIO (getAccountData session userID) <&> \case
...
saveSyncToken token = do
    session <- clientSession
    userID <- myUserID
    liftIO $ setAccountData session userID $
        SyncTokenAccountData { stadSyncToken = token }

mkFilter :: Filter
mkFilter = defaultFilter
    { filterAccountData = Just $
          defaultEventFilter { efNotTypes = Just [ syncTokenAccountDataType ] }
    }
