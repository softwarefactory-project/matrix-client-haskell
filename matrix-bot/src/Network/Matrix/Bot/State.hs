...
class (Monad m) => MonadMatrixBot m where
    ...
    default syncedSince :: (m ~ m' n, MonadTrans m', MonadMatrixBot n)
                        => m (Maybe T.Text)
...
instance (MonadResyncableMatrixBot m, Monoid w)
...
newtype MatrixBotT m a = MatrixBotT (ReaderT MatrixBotEnv m a)
    deriving ( Functor, Applicative, Monad, MonadTrans, MonadIO, MonadCatch
             , MonadMask, MonadThrow, MonadUnliftIO )
...
instance Monad m => MonadResyncableMatrixBot (MatrixBotT m) where
    withSyncStartedAt syncStart (MatrixBotT ac) = MatrixBotT $
